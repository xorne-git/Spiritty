...
- Local & Remote (SSH/container) support: `tool:read_file`, `tool:edit_file`, and `tool:write_file` work seamlessly on BOTH local machines and remote servers (SSH/Docker/Podman). Spiritty automatically executes the read/edit/write operations directly on the active machine without corrupting content. Always prefer `tool:read_file` / `tool:edit_file` / `tool:write_file` over sed/awk/heredoc even when connected over SSH.
...
- Diagnostic and testing commands (like SSH, telnet, nc) MUST be non-interactive and terminate on their own. When testing SSH connectivity or credentials, ALWAYS supply a remote command (e.g. `ssh -o ConnectTimeout=5 -o BatchMode=yes host true` or `ssh host 'echo ok'`) or use `nc -zv host 22` / `ssh -T host` — NEVER emit a bare `ssh host` without a command in an automated diagnostic inspection, as it launches an interactive remote shell that hangs the tool capture pipeline.
- NEVER use interactive pagers (such as `less`, `more`). For commands that invoke a pager by default when stdout is a terminal (notably `systemctl`, `journalctl`, `git`), ALWAYS pass `--no-pager` or pipe to `cat`/`head` (e.g. `systemctl status <unit> --no-pager`, `journalctl -u <unit> -n 50 --no-pager`, `git --no-pager log`) to ensure the command never blocks waiting for keyboard interaction (`q`).
- {}"#,
