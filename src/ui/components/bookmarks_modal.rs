...
pub enum AddHostState {
    None,
    EnteringTarget { input: String, cursor: usize },
    EnteringAlias { target: String, input: String, cursor: usize },
}
...
impl BookmarksModalState {
    ...
    pub fn handle_paste(&mut self, text: String) {
        ...
        match &mut self.add_state {
            AddHostState::EnteringTarget { input, cursor } | AddHostState::EnteringAlias { input, cursor, .. } => {
    ...
    pub fn handle_key(&mut self, key: KeyEvent, hosts_store: &mut HostsStore) -> Option<BookmarksModalAction> {
        ...
        match &mut self.add_state {
            ...
            AddHostState::EnteringAlias { target, input, cursor } => {
                match key.code {
                    KeyCode::Esc => {
                        self.add_state = AddHostState::None;
                        return None;
        ...
        match key.code {
            ...
            KeyCode::Enter => {
                self.entries.get(self.selected_index).map(|entry| BookmarksModalAction::Connect(entry.target.clone()))
            }
    ...
    pub fn render_modal(
        ...
        if let (Some(b_area), Some(ref target)) = (banner_area, &state.active_ssh_target) {
            ...
            let mut banner_spans = vec![
                ...
                Span::styled(
                    if lang == Language::Fr { "Hôte actif : " } else { "Active host: " },
                    Style::default().fg(Color::White).add_modifier(Modifier::BOLD),
                ),
                Span::styled(target.clone(), Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
                Span::raw("  "),
            ];
            banner_spans.extend(key_pill("S", Color::Yellow));
            banner_spans.push(Span::styled(
                if is_bookmarked {
                    if lang == Language::Fr { " Déjà dans vos favoris ★" } else { " Already in bookmarks ★" }
        ...
        match &state.add_state {
            AddHostState::EnteringTarget { input, cursor } => {
                ...
                lines.push(Line::from(vec![
                    Span::styled("[Enter] Suivant    [Esc] Annuler", Style::default().fg(Color::DarkGray)),
                ]));
            ...
            AddHostState::EnteringAlias { target, input, cursor } => {
                let mut lines = Vec::new();
                lines.push(Line::from(Span::styled(
                    if lang == Language::Fr { format!("✎ Donner un alias/nom à '{}' (laisser vide si aucun) :", target) }
                    else { format!("✎ Set an alias/label for '{}' (optional) :", target) },
                    Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD),
                )));
                lines.push(Line::from(""));
                let mut l = vec![Span::styled("❯ ", Style::default().fg(Color::Cyan))];
                l.extend(render_editable_text(input, *cursor, true, "prod-web, vps-db, etc."));
                lines.push(Line::from(l));
                lines.push(Line::from(""));
                lines.push(Line::from(vec![
                    Span::styled("[Enter] Enregistrer    [Esc] Annuler", Style::default().fg(Color::DarkGray)),
                ]));
                let p = Paragraph::new(lines);
                p.render(table_area, buf);
                return;
        ...
        if state.entries.is_empty() {
            ...
            let p = Paragraph::new(Line::from(vec![
                Span::styled(msg, Style::default().fg(Color::DarkGray).add_modifier(Modifier::ITALIC)),
            ])).alignment(Alignment::Center);
            p.render(table_area, buf);
        } else {
            let header_row = Row::new(vec![
                Span::styled("Fav", Style::default().fg(Color::DarkGray).add_modifier(Modifier::BOLD)),
                Span::styled(if lang == Language::Fr { "Alias / Nom" } else { "Alias / Label" }, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                Span::styled(if lang == Language::Fr { "Cible SSH" } else { "SSH Target" }, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            ...
            let rows: Vec<Row> = state.entries.iter().enumerate().skip(offset).take(visible_rows).map(|(idx, entry)| {
                ...
                Row::new(vec![
                    Span::styled(fav_icon, if is_sel { style } else { Style::default().fg(fav_color) }),
        ...
        footer.push(Span::raw(if lang == Language::Fr { " Connecter  " } else { " Connect  " }));

        if state.active_ssh_target.is_some() {
            footer.extend(key_pill("S", Color::Yellow));
            footer.push(Span::raw(if lang == Language::Fr { " Sauvegarder hôte  " } else { " Save host  " }));
        ...
        footer.push(Span::raw(if lang == Language::Fr { " Ajouter  " } else { " Add  " }));

        footer.extend(key_pill("F", Color::Yellow));
        footer.push(Span::raw(if lang == Language::Fr { " Favori ★  " } else { " Favorite ★  " }));

        footer.extend(key_pill("D", Color::Red));
        footer.push(Span::raw(if lang == Language::Fr { " Suppr  " } else { " Delete  " }));
