...
impl HelpModal {
    pub fn render_modal(area: Rect, buf: &mut Buffer, lang: Language) {
        ...
        if modal_width >= 75 && center_x > modal_area.left() + 25 && center_x < modal_area.right().saturating_sub(25) {
            buf.set_string(center_x, modal_area.top(), symbols::line::NORMAL.horizontal_down, border_style);
            for py in (modal_area.top() + 1)..sep_y {
                buf.set_string(center_x, py, symbols::line::NORMAL.vertical, border_style);
            }
            if sep_y > modal_area.top() && sep_y < modal_area.bottom().saturating_sub(1) {
                buf.set_string(center_x, sep_y, symbols::line::NORMAL.horizontal_up, border_style);
            }
        }
        ...
        if modal_width >= 75 {
            ...
            left_lines.push(make_help_row(l_srch, lang.t(I18nKey::HelpDescChatSearch), key_col_w_left));
            ...
            Paragraph::new(left_lines).alignment(Alignment::Left).render(left_area, buf);
            ...
            Paragraph::new(right_lines).alignment(Alignment::Left).render(right_area, buf);
        } else {
            ...
            let lines = vec![
                make_help_row(key_pill("Shift+Tab", Color::Cyan), lang.t(I18nKey::HelpDescToggleFocus), key_col_w),
                make_help_row(key_pill("F3", Color::Green), lang.t(I18nKey::HelpDescAutoApprove), key_col_w),
                make_help_row(key_pill("Ctrl+P", Color::Magenta), lang.t(I18nKey::HelpDescConfigModal), key_col_w),
                make_help_row(key_pill("Ctrl+B", Color::Cyan), lang.t(I18nKey::HelpDescBookmarksModal), key_col_w),
                make_help_row(key_pill("Ctrl+M", Color::Rgb(140, 100, 240)), lang.t(I18nKey::HelpDescMcpModal), key_col_w),
                make_help_row(key_pill("Ctrl+H", Color::LightCyan), lang.t(I18nKey::HelpDescSessionModal), key_col_w),
                make_help_row(key_pill("Ctrl+E", Color::Yellow), lang.t(I18nKey::HelpDescExportSession), key_col_w),
                make_help_row(key_pill("Ctrl+F", Color::Yellow), lang.t(I18nKey::HelpDescChatSearch), key_col_w),
                make_help_row(key_pill("Alt+D", Color::Yellow), lang.t(I18nKey::HelpDescDiagnoseError), key_col_w),
                make_help_row(key_pill("F1", Color::Cyan), lang.t(I18nKey::HelpDescToggleHelp), key_col_w),
                make_help_row(key_pill("Ctrl+Q", Color::Red), lang.t(I18nKey::HelpDescQuit), key_col_w),
            ];

            let content_area = Rect::new(modal_area.left() + 2, inner_y, modal_area.width.saturating_sub(4), inner_height);
            Paragraph::new(lines).alignment(Alignment::Left).render(content_area, buf);
        }
        ...
        Paragraph::new(Line::from(footer)).alignment(Alignment::Center).render(footer_area, buf);
    }
}
